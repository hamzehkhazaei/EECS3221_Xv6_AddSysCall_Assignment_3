...
1. `kernel/syscall.h` - System call number definitions
2. `kernel/syscall.c` - System call dispatch table
3. `kernel/sysproc.c` - System call implementation
4. `user/usys.pl` - Generates the user-space system call stubs
5. `user/user.h` - User-space function declarations
...
2. **Assembly Stub**: `usys.S` that dynamically is generated by `usys.pl` stub puts system call number in register
